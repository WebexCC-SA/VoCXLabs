...
---
Your Attendee ID is: <span class="config-placeholder" data-config-key="attendee_id">###_AttendeeId</span
...
<script>
  // Load used IDs from localStorage (or start empty)
  let usedIds = new Set(JSON.parse(localStorage.getItem("usedIds") || "[]"));
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    genBtn.textContent = "Generate Attendee ID";
    genBtn.style.margin = "10px";

    // Reset button
    const resetBtn = document.createElement('button');
    resetBtn.textContent = "Reset Attendee IDs";
    ...
    document.body.prepend(genBtn);

    // Button actions
    genBtn.addEventListener('click', function() {
      const id = generateAttendeeId();
      if (id) {
        output.textContent = "✅ Assigned Attendee ID: " + id;
      } else {
        output.textContent = "⚠️ All IDs from 101–150 have been assigned.";
      }
    });

    resetBtn.addEventListener('click', function() {
      resetAttendeeIds();
      output.textContent = "🔄 All Attendee IDs have been reset.";
    });
  });
</script>
